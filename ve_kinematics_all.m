...
for i = 1:3
    ...
    hold on;
    for j = 1:n_ve_profiles
        ve_handle = plot(x / 1e3, velocity_profile_store{i, j}, '-r', LineWidth=1.0);
    end
    ...
    hold on;
    for j = 1:n_ve_profiles
        ve_handle = plot(x / 1e3, velocity_perturbation_profile_store{i, j}, '-r', LineWidth=1.0);
    end
    ...
    suptitle(strcat("n = ", string(n_array(i))), fontsize);
end
return;
